...
public:
    ...
    int getMinimumLargestSplitSum(vector<int>& prefixSum, int currIndex, int subarrayCount, int& n, vector<vector<int>>& memo) {
        ...
        for (int i = currIndex; i <= n - subarrayCount; i++) {
            ...
            int largestSplitSum = max(firstSplitSum, 
                                      getMinimumLargestSplitSum(prefixSum, i + 1, subarrayCount - 1, n, memo));
    ...
    int splitArray(vector<int>& nums, int m) {
        ...
        return getMinimumLargestSplitSum(prefixSum, 0, m, n, dp);
    }
};